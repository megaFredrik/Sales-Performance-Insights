...
function share(value,total){return total?Math.round(value/total*100):0}

function renderTable(id,data,m){const sorted=[...data].sort((a,b)=>metricValue(b,m)-metricValue(a,m));const top=sorted[0]?.name;$(id).innerHTML=`<thead><tr><th>Name</th><th>${metricLabel(m)}</th><th>Avg deal</th><th>Share of total</th></tr></thead><tbody>${sorted.map(x=>`<tr><td><strong>${x.name===top?'🏆 ':''}${x.name}</strong></td><td>${formatMetric(metricValue(x,m),m)}</td><td>${money(x.avgDeal)}</td><td>${share(metricValue(x,m),sorted.reduce((s,y)=>s+metricValue(y,m),0))}%</td></tr>`).join('')}</tbody>`}
function renderChart(canvasId,data,m){const ctx=$(canvasId);if(charts[canvasId])charts[canvasId].destroy();const sorted=[...data].sort((a,b)=>metricValue(b,m)-metricValue(a,m));charts[canvasId]=new Chart(ctx,{type:'bar',data:{labels:sorted.map(x=>x.name),datasets:[{label:metricLabel(m),data:sorted.map(x=>metricValue(x,m))}]},options:{responsive:true,plugins:{legend:{display:false},tooltip:{callbacks:{label:c=>`${metricLabel(m)}: ${formatMetric(c.raw,m)}`}}},scales:{y:{beginAtZero:true,ticks:{callback:v=>m==='deals'?v:money(v)}}}}})}

function buildInsights(m,reps,regions,products,totalRev,totalDeals){
 const bestRep=[...reps].sort((a,b)=>metricValue(b,m)-metricValue(a,m))[0];
 const worstRep=[...reps].sort((a,b)=>metricValue(a,m)-metricValue(b,m))[0];
 const bestRegion=[...regions].sort((a,b)=>metricValue(b,m)-metricValue(a,m))[0];
 const weakestRegion=[...regions].sort((a,b)=>metricValue(a,m)-metricValue(b,m))[0];
 const bestProduct=[...products].sort((a,b)=>b.revenue-a.revenue)[0];
 const secondRep=[...reps].sort((a,b)=>metricValue(b,m)-metricValue(a,m))[1];
 const gap=secondRep?Math.round((metricValue(bestRep,m)-metricValue(secondRep,m))/Math.max(metricValue(secondRep,m),1)*100):0;
 $('topPerformer').textContent=bestRep?.name||'—';
 $('topPerformerMeta').textContent=bestRep?`${formatMetric(metricValue(bestRep,m),m)} · ${metricLabel(m)}`:'';
 const summary=`${bestRep.name} leads the team on ${metricLabel(m).toLowerCase()} with ${formatMetric(metricValue(bestRep,m),m)}${gap?`, ${gap}% ahead of the next rep`:''}. ${bestRegion.name} is the strongest region on the selected metric, while ${bestProduct.name} contributes the most revenue. Overall, the team closed ${num(totalDeals)} deals worth ${money(totalRev)}.`;
 $('executiveSummary').textContent=summary;
 const attention=[];
 if(worstRep) attention.push(`<strong>${worstRep.name}</strong> is currently lowest on ${metricLabel(m).toLowerCase()} (${formatMetric(metricValue(worstRep,m),m)}). Review pipeline coverage and recent deal activity.`);
 if(weakestRegion) attention.push(`<strong>${weakestRegion.name}</strong> trails the other regions on the selected metric. Check territory mix, pipeline health and capacity.`);
 if(secondRep&&gap>=25) attention.push(`<strong>${bestRep.name}</strong> is ${gap}% ahead of the next rep. Capture and share the practices behind the performance.`);
 if(bestProduct) attention.push(`<strong>${bestProduct.name}</strong> is the largest revenue contributor at ${money(bestProduct.revenue)}. Consider whether more capacity or cross-sell focus is justified.`);
 $('attentionList').innerHTML=attention.slice(0,4).map(x=>`<li>${x}</li>`).join('');
}

function update(){if(!rows.length)return;const m=$('metricSelect').value,totalRev=rows.reduce((s,r)=>s+r.Revenue,0),totalDeals=rows.reduce((s,r)=>s+r.DealsClosed,0);$('totalRevenue').textContent=money(totalRev);$('totalDeals').textContent=num(totalDeals);$('avgDealSize').textContent=money(totalDeals?totalRev/totalDeals:0);$('revenueMeta').textContent='Across uploaded data';$('dealsMeta').textContent='Closed deals';$('avgMeta').textContent='Revenue ÷ deals';const reps=groupBy('Rep'),regions=groupBy('Region'),products=groupBy('Product');renderChart('regionChart',regions,m);renderChart('repChart',reps,m);renderChart('productChart',products,m);renderTable('regionTable',regions,m);renderTable('repTable',reps,m);renderTable('productTable',products,m);buildInsights(m,reps,regions,products,totalRev,totalDeals);$('dashboard').classList.remove('hidden');}
